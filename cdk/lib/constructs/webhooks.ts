...
export class Webhooks extends Construct {
  ...
  constructor(scope: Construct, id: string, props: WebhooksProps) {
    ...
    // Create default stage with auto-deploy (no explicit deployment needed)
    const stage = new apigatewayv2.CfnStage(this, 'WebhooksStage', {
      apiId: this.api.ref,
      stageName: '$default',
      autoDeploy: true,
      description: 'Auto-deploy stage for webhooks API'
    });

    // Map custom domain to API
    const apiMapping = new apigatewayv2.CfnApiMapping(this, 'WebhooksAPIMappingV2', {
      apiId: this.api.ref,
