                ...
                for (const cot of cots) {
                    ...
                    for (const client of (this.config.wsClients.get(String(conn.id)) || [])) {
                        if (client.format == 'geojson') {
                            // NOTE: upstream logs `console.log(JSON.stringify(feat))` for
                            // DataSyncMissionsList chats here. Deliberately omitted (TAK-NZ patch
                            // 066): it writes full mission-chat payloads, message text included,
                            // to the server log.
                                ...
                                client.ws.send(JSON.stringify({
                                    ...
                                    data: {
                                        ...
                                        from: {
                                            ...
                                            // Upstream has since adopted this too; keep exactly one.
