...
export default class IconManager {
    ...
    private async resolveImage(id: string): Promise<void> {
        ...
        if (id.startsWith('2525C:') || id.startsWith('2525D:') || id.startsWith('2525E:')) {
            ...
            // Skip LINZ sprite images - these come from the `linz-topographic`
            // named sprite and MapLibre resolves them itself. Routing them to
            // loadIconsetImage would look for a Dexie iconset called
            // 'linz-topographic', which does not exist.
            if (!id.startsWith('linz-topographic:')) {
    ...
    private async decodeIconBlob(blob: Blob): Promise<ImageBitmap> {
        ...
        try {
            ...
            // Already within the target size - use as-is.
            if (raw.width <= SVG_RENDER_WIDTH && raw.height <= SVG_RENDER_WIDTH) {
                return raw;
            }

            // Scale so the longest edge equals SVG_RENDER_WIDTH, preserving aspect
            // ratio - the same constraint applied to SVG icons. Without this an
            // oversized source raster renders as a screen-filling marker.
