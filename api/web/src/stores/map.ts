    ...
    actions: {
        ...
        _teardown: async function(): Promise<void> {
            ...
            if (currentWorker && currentRawWorker) {
                try {
                    // Bounded on purpose: init() waits for this teardown before
                    // building a new map, so an unanswerable RPC to a wedged worker
                    // would become a permanent loading spinner on the next mount.
                    // terminate() below is the cleanup that actually matters.
        ...
        init: async function(container: HTMLElement): Promise<boolean> {
            ...
            // issues. The sprite is loaded directly from the LINZ CDN, which is a
            // public endpoint (Access-Control-Allow-Origin: *) and must be listed
            // in CLOUDTAK_TILE_ORIGINS so the CSP connect-src allows it.
            // Image references in the LINZ style JSON files are prefixed with
            // 'linz-topographic:' to match the IDs that MapLibre assigns to
            // non-default named sprites.
            sprites.push({
                id: 'linz-topographic',
                url: 'https://basemaps.linz.govt.nz/v1/sprites/topographic'
            });
