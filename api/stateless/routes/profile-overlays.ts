...
export default async function router(schema: Schema, config: ConfigStateless) {
    ...
    await schema.post('/profile/overlay', {
        ...
        try {
            ...
            // TAK-NZ: a repeat POST for an overlay we already have unhides it and
            // returns it, rather than erroring or creating a duplicate. The
            // frontend re-POSTs the default basemap on every map init, so this has
            // to stay idempotent.
            //
            // Deliberately ordered BEFORE upstream's single-basemap check below:
            // re-POSTing the *same* basemap URL returns 200 here, while a POST for
            // a *different* basemap falls through and correctly hits upstream's
            // 400.
            if (req.body.mode !== 'mission' && req.body.url) {
                ...
                if (existing.total > 0) {
                    ...
                    return res.json(await augmentOverlay(config, overlay, user));
                }
            }
