...
| 026 | `api/lib/authentik-provider.ts` | Import path fixes (`./fetch.js` → global fetch, `./external.js` → `./interface-user.js`), ESLint cleanup; all methods were already in upstream v13. **v13 port addition (Jul 2026):** adds `enrollUserCertificate(username, takServerUrl)` — sets a random temporary password on the Authentik user account, generates a TAK client certificate via `APIAuthPassword` + `Credentials.generate()`, then revokes the temp password in a `finally` block | ✅ Applied & valid |
