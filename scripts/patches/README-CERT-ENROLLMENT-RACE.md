...
**`api/lib/authentik-provider.ts`** — wrap the set-password / TAK-login / revoke-password
...
**`api/routes/login.ts`** — give cert enrollment and attribute sync independent try/catch
...
- Distinct from the DB-commit-overwrite race originally described above this section
  (patch 052, now superseded — see the note at the top of this file and in `PATCH_AUDIT.md`).
